...
def memb_prob(ID, memb_prob, MP_cut=(.9,), ID_col=False):
    """
...
def resultsProcess(data, Nruns=50, ID_col=False):
    ...
    if ID_col:
        ...
        ID_name, prob0, probs_c, prob_f = 'field', 'class', 'class.', 'probability'

    metrics = []
    # Store first class
    probs = [data[prob0].data]
    print("{} -> C={:.3f}, P={:.3f}, M={:.3f}, L={:.3f}".format(
        0, *memb_prob(data[ID_name], probs[0], ID_col=ID_col)[:4]))
    for c in range(1, Nruns):
        probs.append(data[probs_c + str(c)].data)
        pr_mean = np.mean(probs, 0)
        C, P, M, L = memb_prob(data[ID_name], pr_mean, ID_col=ID_col)[:4]
        print("{} -> C={:.3f}, P={:.3f}, M={:.3f}, L={:.3f}".format(
            c, C, P, M, L))
        metrics.append([C, P, M, L])

    metrics_final = memb_prob(
...
# plt.title("R Sol 1, 50")



# plt.subplot(241)
# resultsProcess(
#     ascii.read("output/up-0.80_0.49_0.49_0.33_0.29_R_50_1.dat"), ID_col=True)
# plt.title("R 1, 50")

# plt.subplot(242)
# resultsProcess(
#     ascii.read("output/up-0.80_0.49_0.49_0.33_0.29_Subl_50_1.dat"),
#     ID_col=True)
# plt.title("R 2, 50")

# plt.subplot(243)
# resultsProcess(
#     ascii.read("output/0.80_0.49_0.49_0.33_0.29_R_R.dat"), ID_col=True)
# plt.title("Python R R, 50")

# plt.subplot(244)
# resultsProcess(
#     ascii.read("output/0.80_0.49_0.49_0.33_0.29_R_PR.dat"), ID_col=True)
# plt.title("Python R PR, 50")

# plt.subplot(245)
# resultsProcess(
#     ascii.read("output/0.80_0.49_0.49_0.33_0.29_R_P.dat"), ID_col=True)
# plt.title("Python R P, 50")

# plt.subplot(246)
# resultsProcess(
#     ascii.read("output/0.80_0.49_0.49_0.33_0.29_P_R.dat"), ID_col=True)
# plt.title("Python P R, 50")

# plt.subplot(247)
# resultsProcess(
#     ascii.read("output/0.80_0.49_0.49_0.33_0.29_P_PR.dat"), ID_col=True)
# plt.title("Python P PR, 50")

# plt.subplot(248)
# resultsProcess(
#     ascii.read("output/0.80_0.49_0.49_0.33_0.29_P_P.dat"), ID_col=True)
