...
import numpy as np
from . import inner
from .extras import reSampleData, dimReduc, train_gmm


def main(
    ID, xy, data, data_err, resampleFlag, PCAflag, PCAdims, clust_method,
        RK_rad, RK_vals, Kest, C_thresh, clust_params, cl_method_pars, prfl):
    """
    ...
    while True:
        ...
        C_masks, N_clusts = inner.main(
            clust_xy, clust_data, clust_method, RK_rad, RK_vals, Kest,
            C_thresh, clust_params, cl_method_pars, prfl)
        ...
        if clust_data[msk_all].shape[0] < int(clust_params['N_membs']):
            print(" N_stars<{:.0f} Breaking".format(int(
                clust_params['N_membs'])), file=prfl)
            break
    ...
    # probs = outlierRjct(clust_xy, probs)
    probs = train_gmm(clust_xy, probs)
